...
# 남녀가 말하는 음성 파일 입력 
audio_file = 'E:\\nmb\\nmb_data\\STT_multiple_speaker_temp\\0521_nowav\\korea_multi_t12.wav'
...
for i, chunk in enumerate(audio_chunks): 
    speaker_stt = []   
    out_file = folder_path + "\\"+ "chunk.wav"    # wav 파일 생성하고 작업 끝나면 지우기
    ...
    try : 
        ...
        if len(y) >= 22050*5 : # 5초 이상이라면,
            ...
            out_file_over5s = folder_path + "\\"+ "chunk_over_5s.wav"
            audio_copy.export(out_file_over5s , format='wav')
            y_copy, sampling_rate = librosa.load(out_file_over5s, sr=22050)
